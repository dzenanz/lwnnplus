...
void network::save (const char *filename) const
{
  ...
  file = fopen (filename, "wb");//added b for binary. see line 622 (this is line 589)
  if (file == NULL)
    {
...
network::do_load (const char *filename)
{
  ...
  file = fopen (filename, "rb"); //b (for binary) missing in original version (0.998) of lwn++ (causes fatal failures on CR/LF in input file)
  if (file == NULL)
    {
...
/****************************************
 * Loading/saving from/to in-memory arrays
 ****************************************/

/*!\brief Constructor. Load network from a binary array.
 */
network::network (const char *binary_file_content, const int len)
{
  int no_of_layers, l, nu, funct, *arglist;

  const char *bin=binary_file_content;
  int i=0;

  /* read function */
  memcpy(&funct, &bin[i], sizeof(int)); i+=sizeof(int);

  if (i>=len) {
    throw runtime_error("network: No enough information in array");
  }

    /* read network dimensions */
  memcpy(&no_of_layers, &bin[i], sizeof(int)); i+=sizeof(int);  
  ...
  memcpy(arglist, &bin[i], sizeof(int)*no_of_layers); i+=sizeof(int)*no_of_layers;
  if (i>=len) {
	free(arglist);
    throw runtime_error("network: No enough information in array");
  }
  ...
  /* read network constants */
  memcpy(&momentum, &bin[i], sizeof(int)); i+=sizeof(int);
  memcpy(&learning_rate, &bin[i], sizeof(int)); i+=sizeof(int);
  memcpy(&global_error, &bin[i], sizeof(int)); i+=sizeof(int);
  ...
  for (l = 1; l < no_of_layers; l++)
  {
    for (nu = 0; nu < layer[l].no_of_neurons; nu++)
	{
	  memcpy(layer[l].neuron[nu].weight, &bin[i], sizeof(float)*layer[l - 1].no_of_neurons);
	  i+=sizeof(float)*layer[l - 1].no_of_neurons;
	}
  }

  if (i>len) {
    throw runtime_error("network: Array too short");
  }
}


/*!\brief Save network to memory array
 * \param len Placeholder for resulting length.
 */
char *network::save (int& len)
{
  int l, nu;

  len=sizeof(int)*(2+no_of_layers)+sizeof(float)*3;
  for (l = 1; l < no_of_layers; l++)
	  len+=layer[l].no_of_neurons*layer[l - 1].no_of_neurons*sizeof(float);
  char *binary_file_content=(char *)malloc(len);
  char *curr=binary_file_content;

  /* write activation funct. and no. of layers */
  memcpy(curr, &activation, sizeof(int)); curr+=sizeof(int);
  memcpy(curr, &no_of_layers, sizeof(int)); curr+=sizeof(int);

  /* write network dimensions */
  for (l = 0; l < no_of_layers; l++)
  {
     memcpy(curr, &(layer[l].no_of_neurons), sizeof(int)); curr+=sizeof(int);
  }
  ...
  memcpy(curr, constants, sizeof(constants)); curr+=sizeof(constants);
  ...
  for (l = 1; l < no_of_layers; l++)
  {
     for (nu = 0; nu < layer[l].no_of_neurons; nu++)
	 {
		memcpy(curr, layer[l].neuron[nu].weight, sizeof(float)*layer[l - 1].no_of_neurons); curr+=sizeof(float)*layer[l - 1].no_of_neurons;
	 }
  }
...
/*!\brief Adds one neuron in output layer with random weights
 *
 * Adds one additional output (usefull for classification purposes)
 */
void network::add_output()
{
	layer[no_of_layers-1].no_of_neurons++;

	layer[no_of_layers-1].neuron=(neuron_t *)realloc(layer[no_of_layers-1].neuron, layer[no_of_layers-1].no_of_neurons*sizeof(neuron_t));
	output_layer->neuron[output_layer->no_of_neurons-1].weight=(float *) calloc (layer[no_of_layers-2].no_of_neurons, sizeof (float));
	output_layer->neuron[output_layer->no_of_neurons-1].delta=(float *) calloc (layer[no_of_layers-2].no_of_neurons, sizeof (float));
	output_layer->neuron[output_layer->no_of_neurons-1].sumdeltas=(float *) calloc (layer[no_of_layers-2].no_of_neurons, sizeof (float));

	for (int nl = 0; nl < layer[no_of_layers-2].no_of_neurons; nl++)
		output_layer->neuron[output_layer->no_of_neurons-1].weight[nl] = RANDOM_LARGE_WEIGHTS_LOGISTIC * ((float) rand () / RAND_MAX - 0.5);
