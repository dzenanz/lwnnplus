...
public:
  ...
  /*!\brief Constructor. Load network from in-memory binary array.
   * \param binary_file_content Pointer to first byte of array with file content
   */
	network (const char *binary_file_content, const int len);

  /*!\brief Saves network to in-memory binary array.
   * Caller should free the memory allocated by this function
   */
	char *save (int& len);
